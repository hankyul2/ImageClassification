...
def convert_to_dataloader(datasets, batch_size, num_workers, shuffle=True, sampler_fn=None):
    return [torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=(shuffle if not sampler_fn else False), num_workers=num_workers,
                                        drop_last=True, sampler=(sampler_fn(ds) if sampler_fn else None)) for ds in datasets]
